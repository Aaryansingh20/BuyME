...
import type { ShopProduct } from "@/public/data/shop"
import { ProductReviews } from "@/components/ui/product-reviews"
import { Price } from "@/components/ui/price"

// Heavy three.js viewer — loaded only when a product actually has a 3D model.
...
export function ProductDetail({ product, related }: { product: ShopProduct; related: ShopProduct[] }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?slug=${encodeURIComponent(slug)}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => d && typeof d.average === "number" && setAverageRating(d.average))
      .catch(() => {})
  }, [slug])
              ...
              <div className="relative overflow-hidden rounded-lg bg-zinc-800" style={{ aspectRatio: "1 / 1" }}>
                ...
                {product.model3d && (
                  <button
                    ...
                    className={`absolute bottom-3 right-3 z-10 flex items-center gap-2 rounded-full bg-black/70 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-white backdrop-blur transition-colors hover:bg-black ${
            ...
            <div className="w-full space-y-6 md:w-1/2">
              <div>
                ...
                <div className="space-y-2">
                  ...
                  <Price amount={displayPrice} className="block text-2xl font-bold text-white" />
                </div>
              </div>
                ...
                {activeTab === "shipping" && (
                  <div className="space-y-4">
                    <div>
                      ...
                      <ul className="list-disc space-y-1 pl-5">
                        <li>Free standard shipping on orders over <Price amount={100} /></li>
              ...
              {related.map((item) => (
                <Link key={item.slug} href={`/product/${item.slug}`} className="group">
                  ...
                  <div className="flex items-center justify-between">
                    <Price amount={item.price} className="text-white" />
                  </div>
                </Link>
              ))}
            </div>
