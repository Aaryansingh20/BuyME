...
type ProfileInfoProps = {
  user: User;
  setUser: (updatedUser: Partial<User>) => void; // Accept partial updates
};
...
export function ProfileInfo({ user, setUser }: ProfileInfoProps) {
  ...
  return (
    <Card className="bg-zinc-900 border-zinc-800 shadow-lg">
      <CardHeader>
        <CardTitle className="text-2xl text-zinc-100">Personal Information</CardTitle>
        <CardDescription className="text-zinc-400">Update your personal details here</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex items-center space-x-6">
          <div className="relative">
            <Avatar className="w-24 h-24 bg-zinc-800 border-2 border-zinc-700">
              <AvatarImage src={editedUser.avatar} alt={editedUser.name} />
              <AvatarFallback className="text-2xl text-zinc-100">
                {editedUser.name
                  .split(" ")
                  .map((n) => n[0])
                  .join("")}
          ...
          <div>
            <h2 className="text-2xl font-semibold text-zinc-100">{editedUser.name}</h2>
            <p className="text-zinc-400">{editedUser.email}</p>
          </div>
        </div>
        {isEditing ? (
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                ...
                <p className="text-zinc-100 font-medium">{user.phone}</p>
              </div>
              <div>
                <Label className="text-zinc-300">Address</Label>
                <p className="text-zinc-100 font-medium">{user.address}</p>
              </div>
            </div>
            <Button
              className="bg-zinc-100 text-zinc-900 hover:bg-zinc-200"
